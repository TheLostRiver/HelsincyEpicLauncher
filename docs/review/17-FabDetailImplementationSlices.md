...
| S5 | 更多内容的数据质量提升 | 已完成 | 降低“同作者更多内容”误匹配 |
...
| S5-A | S5 | 已完成 | 更多内容去重与排除自身 |
| S5-B | S5 | 已完成 | 更多内容排序稳定化 |
| S5-C | S5 | 已完成 | 更多内容空结果与退化策略整理 |
...
- 已完成结果：
  - `FilterRelatedItems(...)` 会继续排除当前 `AssetId`
  - 结果会先按 `AssetId` 去重，再按规范化标题去重
  - 去重逻辑保持原始搜索顺序，不引入新的排序规则
...
- 已完成结果：
  - “更多内容”在去重后会显式按 `Rating` 降序排序
  - 同分结果按规范化标题升序，再按 `Price`、`AssetId` 打平
  - 排序规则固定后，搜索路径与 owned 回退路径都能得到稳定顺序
...
- 已完成结果：
  - “更多内容”无结果时保持隐藏区块，不显示异常空态
  - 在线搜索失败或无匹配时会显式回退到已拥有资产路径
  - 若回退后仍无匹配，页面保持稳定且仅记录日志，不抛出异常
