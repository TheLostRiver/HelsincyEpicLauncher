...
| S3-A | S3 | 未开始 | 页面离开前保存滚动位置 |
| S3-B | S3 | 未开始 | 页面恢复后回滚到上次位置 |
| S3-C | S3 | 未开始 | 点击详情前先固化最新快照 |
...
2. `S3-B`，页面恢复后回滚位置
3. `S3-C`，点击详情前固化最新快照
4. 再进入 `S4-A`，定义快照年龄分类辅助逻辑

原因：

1. `S2` 已经完成，`FabLibraryViewModel` 现在具备完整的恢复入口和成功路径写回能力。
2. `S3-A` 到 `S3-C` 仍然可以保持很小的单步推进，不必一次改完整个返回体验。
3. 完成 `S3` 后，才适合继续做 `S4` 的快照年龄与后台刷新策略。
