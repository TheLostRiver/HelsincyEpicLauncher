...
- 发现实现和文档不一致，需要后续决策。
- 当前会话准备结束。
- AI 无法读取精确额度，因此以保守上下文风险信号代替额度检测；一旦判断可能接近限额，必须记录并停止。

### 0.1.1 上下文风险信号

出现以下任一信号时，视为额度风险已经触发：

- 当前任务已经读取或修改多个关键文件，继续执行需要跨更多文件。
- 当前任务尚未完成，但状态已经复杂到需要解释“已做/未做/下一步”。
- 即将修改 Contracts、DI、启动流程、后台生命周期或下载管线。
- 工具输出过长，或出现测试失败、构建失败、设计分歧，需要多轮排查。
- 执行者主观判断继续推进可能导致压缩后丢失关键上下文。

触发后必须执行：

1. 更新 `docs/SessionContextRecord.md`，写清当前任务、已改文件、未验证内容、最近命令、失败摘要和下一步唯一动作。
2. 停止继续修改代码或文档。
3. 等待用户发送继续指令，或等待上下文压缩后按恢复协议继续。
