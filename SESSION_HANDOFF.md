...
## 2026-04-26 文档增量
- 完成内容：新增 [docs/review/19-FabLibraryWarmResumeImplementationSlices.md](docs/review/19-FabLibraryWarmResumeImplementationSlices.md)，把 Fab 列表页热恢复方案拆成细粒度原子任务；同时更新 [docs/review/18-FabLibraryWarmResumeStrategy.md](docs/review/18-FabLibraryWarmResumeStrategy.md) 与 [docs/06-ModuleDefinitions/FabLibrary.md](docs/06-ModuleDefinitions/FabLibrary.md) 的入口引用；已完成 `S1-A`，新增 [src/Launcher.Presentation/Modules/FabLibrary/FabLibrarySessionSnapshot.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibrarySessionSnapshot.cs)；已完成 `S1-B`，新增 [src/Launcher.Presentation/Modules/FabLibrary/IFabLibrarySessionStateStore.cs](src/Launcher.Presentation/Modules/FabLibrary/IFabLibrarySessionStateStore.cs)；已完成 `S1-C`，新增 [src/Launcher.Presentation/Modules/FabLibrary/InMemoryFabLibrarySessionStateStore.cs](src/Launcher.Presentation/Modules/FabLibrary/InMemoryFabLibrarySessionStateStore.cs)；已完成 `S1-D`，更新 [src/Launcher.Presentation/DependencyInjection.cs](src/Launcher.Presentation/DependencyInjection.cs) 注册会话 Store；已完成 `S2-A`，更新 [src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs) 注入 Session Store 并补恢复守卫字段；已完成 `S2-B`，新增快照卡片恢复辅助方法；已完成 `S2-C`，把恢复入口接入 `LoadAsync`；已完成 `S2-D`，把成功查询路径统一写回快照；已完成 `S3-A`，页面离开前写回滚动位置；已完成 `S3-B`，页面恢复后回滚到上次位置；已完成 `S3-C`，点击详情前固化最新快照；已完成 `S4-A`，新增 [src/Launcher.Presentation/Modules/FabLibrary/FabLibrarySnapshotAgePolicy.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibrarySnapshotAgePolicy.cs) 统一定义 `Fresh / Warm / Stale` 年龄分类与 `30s / 5m` 阈值；已完成 `S4-B`，更新 [src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs) 使 `Fresh` 快照恢复后立即显示已恢复列表，不再因分类加载继续停留在骨架屏；已完成 `S4-C`，继续更新 [src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs) 让 `Warm` 快照在列表已可见的前提下后台静默刷新第一页；已完成 `S4-D`，继续更新 [src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs) 让 Warm 静默刷新在失败场景下保留当前已恢复列表，并通过现有通知机制给轻提示；已完成 `S4-E`，继续更新 [src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs) 让 `Stale` 快照直接跳过恢复并回到完整加载路径，至此 `S4` 已整体完成；已完成 `S5-A`，更新 [src/Launcher.Presentation/Modules/FabLibrary/FabLibrarySessionSnapshot.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibrarySessionSnapshot.cs) 与 [src/Launcher.Presentation/Modules/FabLibrary/InMemoryFabLibrarySessionStateStore.cs](src/Launcher.Presentation/Modules/FabLibrary/InMemoryFabLibrarySessionStateStore.cs) 把会话快照统一限制为最多 `3` 页、`60` 条摘要；已完成 `S5-B`，更新 [src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs) 使用当前登录 `AccountId` 写入并校验 `AccountScopeKey`，避免跨账号恢复旧快照；已完成 `S5-C`，继续更新 [src/Launcher.Presentation/Modules/FabLibrary/InMemoryFabLibrarySessionStateStore.cs](src/Launcher.Presentation/Modules/FabLibrary/InMemoryFabLibrarySessionStateStore.cs) 与 [src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibraryViewModel.cs) 为会话快照补齐 `Save / Clear / Trim / ScopeMismatch` 结构化日志，并明确手动刷新、账号作用域不匹配、会话过期三条显式清理路径，至此 `S5` 已整体完成；已完成 `S6-A`，更新 [src/Launcher.Application/Modules/Settings/Contracts/ConfigModels.cs](src/Launcher.Application/Modules/Settings/Contracts/ConfigModels.cs)、[src/Launcher.Application/Modules/Settings/Contracts/ISettingsReadService.cs](src/Launcher.Application/Modules/Settings/Contracts/ISettingsReadService.cs) 与 [src/Launcher.Application/Modules/Settings/Contracts/ISettingsCommandService.cs](src/Launcher.Application/Modules/Settings/Contracts/ISettingsCommandService.cs)，为 Fab 预热开关补齐设置契约入口；已完成 `S6-B`，继续更新 [src/Launcher.Infrastructure/Settings/UserSettings.cs](src/Launcher.Infrastructure/Settings/UserSettings.cs) 与 [src/Launcher.Infrastructure/Settings/SettingsService.cs](src/Launcher.Infrastructure/Settings/SettingsService.cs) 把 `FabLibraryConfig` 接入现有 `user.settings.json` 持久化模型、变更广播与默认值管线；已完成 `S6-C`，更新 [src/Launcher.Presentation/Modules/Settings/SettingsViewModel.cs](src/Launcher.Presentation/Modules/Settings/SettingsViewModel.cs) 暴露 `AutoWarmOnStartup` 绑定属性，并补上 `SaveFabLibraryConfigAsync()` 保存命令；已完成 `S6-D`，更新 [src/Launcher.Presentation/Modules/Settings/SettingsPage.xaml](src/Launcher.Presentation/Modules/Settings/SettingsPage.xaml) 与 [src/Launcher.Presentation/Modules/Settings/SettingsPage.xaml.cs](src/Launcher.Presentation/Modules/Settings/SettingsPage.xaml.cs) 在设置页“通用”区块增加 Fab 预热开关 UI，至此 `S6` 已整体完成；已完成 `S7-A`，新增 [src/Launcher.Presentation/Modules/FabLibrary/FabLibraryWarmupCoordinator.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibraryWarmupCoordinator.cs) 并更新 [src/Launcher.Presentation/DependencyInjection.cs](src/Launcher.Presentation/DependencyInjection.cs)，把 Fab 启动预热收敛为独立协调器服务，并接入 Presentation DI；已完成 `S7-B`，更新 [src/Launcher.App/App.xaml.cs](src/Launcher.App/App.xaml.cs) 把协调器挂到启动 Phase 3 的后台服务流程中，并将 [src/Launcher.Presentation/Modules/FabLibrary/FabLibraryWarmupCoordinator.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibraryWarmupCoordinator.cs) 提升为宿主可直接调用的公开服务；本轮已完成 `S7-C`，继续更新 [src/Launcher.Presentation/Modules/FabLibrary/FabLibraryWarmupCoordinator.cs](src/Launcher.Presentation/Modules/FabLibrary/FabLibraryWarmupCoordinator.cs)，把 `disabled / unauthenticated / offline / fresh_snapshot` 统一收敛成静默跳过，并保持预热失败只写日志不影响启动体验
- 当前重点：继续按 [docs/review/19-FabLibraryWarmResumeImplementationSlices.md](docs/review/19-FabLibraryWarmResumeImplementationSlices.md) 执行 `S8-A`，为 Session Store / 年龄策略补单测
- 当前建议：首期主路径只做“快照恢复 + SWR + Phase 3 预热协调器”，暂不把 `NavigationCacheMode.Required` 当成前置条件；页面缓存仅作为 `S9` 可选实验切片
- 本轮验证：`dotnet build src/Launcher.Presentation/Launcher.Presentation.csproj --no-restore` 与 `dotnet build src/Launcher.App/Launcher.App.csproj --no-restore` 已通过；未执行 `dotnet test`
